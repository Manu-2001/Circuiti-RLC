...
int Intersezione(misura& X1, misura& X2, misura& Y1, misura& Y2, TF1* const& f,
  ...
  double fp0 =
      eval(c / (a * sqrt(delta)) + (-b + sqrt(delta)) / (2 * pow(a, 2)));
  double fp1 = eval((b / sqrt(delta) - 1) / (2 * a));
  double fp2 = 1 / sqrt(delta);
  double gp0 =
      eval(c / (a * sqrt(delta)) + (-b + sqrt(delta)) / (2 * pow(a, 2)));
  double gp1 = eval((1 - b / sqrt(delta)) / (2 * a));
  double gp2 = 1 / sqrt(delta);

  if (quad) {
    X1.p = sqrt(pow(fp0 * dfp[0], 2) + pow(fp1 * dfp[1], 2) +
