...
Double_t fw(Double_t* x, Double_t* par) {
  return par[0] / sqrt(1 + pow(2 * M_PI * par[1] * x[0], 2));
}

Double_t ft(Double_t* x, Double_t* par) {
  return par[0] / sqrt(1 + 1 / pow(2 * M_PI * par[1] * x[0], 2));
}

void fit (){
  TFile* myFile = new TFile("DataFit2.root", "RECREATE");

  TGraphErrors* Tweeter = new TGraphErrors("tweeter.txt", "%lg%lg%lg%lg", "");
  TGraphErrors* Woofer = new TGraphErrors("woofer.txt", "%lg%lg%lg%lg", "");

  TF1* fwl = new TF1("fwl", fw, 1000., 15000., 2);
  TF1* ftc = new TF1("ftc", ft, 1000., 15000., 2);
